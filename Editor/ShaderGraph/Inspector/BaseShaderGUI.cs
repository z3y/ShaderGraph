...
namespace z3y.BuiltIn.ShaderGraph
{
    public class BuiltInBaseShaderGUI : ShaderGUI
    {
        ...
        override public void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            ...
            if (targetMat.HasProperty("_AlphaToMask") && targetMat.HasProperty(Property.AlphaClip()))
            {
                targetMat.SetFloat("_AlphaToMask", targetMat.GetFloat(Property.AlphaClip()));
            }
        ...
        private void DrawAdvancedOptionalProperty(Material material, string name)
        {
            var prop = FindProperty(name, m_Properties, false);
            if (prop != null) m_MaterialEditor.ShaderProperty(prop, prop.displayName);
        }

        protected virtual void DrawAdvancedOptions(Material material)
        {
            DrawAdvancedOptionalProperty(material, "_BakeryMonoSH");
            DrawAdvancedOptionalProperty(material, "_BicubicLightmap");
            DrawAdvancedOptionalProperty(material, "_LightmappedSpecular");
            DrawAdvancedOptionalProperty(material, "_NonLinearLightMapSH");
            DrawAdvancedOptionalProperty(material, "_NonLinearLightProbeSH");
            DrawAdvancedOptionalProperty(material, "_GeometricSpecularAAToggle");
            DrawAdvancedOptionalProperty(material, "_AnisotropyToggle");

            DrawAdvancedOptionalProperty(material, "_SpecularHighlights");
            DrawAdvancedOptionalProperty(material, "_GlossyReflections");
            ...
            {
                m_MaterialEditor.LightmapEmissionFlagsProperty(0, false);
            }
